...
export default function AuthPage({ lang }: { lang: Lang }) {
  ...
  return (
    <div className="min-h-screen bg-[#e9f3f5] px-4 py-10 text-slate-900">
      <div className="mx-auto max-w-md">
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-semibold text-blue-700"><ArrowLeft className="h-4 w-4" /> {t.backToMedSutra}</Link>
        <div className="mt-6 rounded-[28px] border border-white bg-white/90 p-7 shadow-xl shadow-blue-100/50 backdrop-blur">
          ...
          <h1 className="mt-5 text-2xl font-black text-[#1a3a6b]">{mode === 'signup' ? t.createYourAccount : mode === 'reset' ? t.resetYourPassword : t.welcomeBack}</h1>
          <p className="mt-2 text-sm leading-6 text-slate-600">{t.authSubtitle}</p>
          <form onSubmit={submit} className="mt-6 space-y-4">
            {mode === 'signup' && <Field label="Name" value={name} onChange={setName} icon={<UserRound className="h-4 w-4" />} />}
            <Field label={t.emailAddress} type="email" value={email} onChange={setEmail} icon={<Mail className="h-4 w-4" />} />
            {mode !== 'reset' && <Field label={t.passwordLabel} type="password" value={password} onChange={setPassword} icon={<LockKeyhole className="h-4 w-4" />} hint={mode === 'signup' ? t.passwordHint : undefined} />}
            ...
            <button disabled={submitting} className="w-full rounded-2xl bg-blue-600 px-4 py-3.5 font-bold text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700 disabled:opacity-60">{submitting ? t.pleaseWait : mode === 'signup' ? t.createSecureAccount : mode === 'reset' ? t.requestResetLink : t.signIn}</button>
          </form>
          {mode !== 'reset' && <div className="mt-5 text-center text-sm text-slate-600">{mode === 'signup' ? t.alreadyHaveAccount : t.newToMedSutra} <button onClick={() => setMode(mode === 'signup' ? 'signin' : 'signup')} className="font-bold text-blue-700">{mode === 'signup' ? t.signIn : t.createAnAccount}</button></div>}
          {mode !== 'reset' && <button onClick={() => setMode('reset')} className="mt-3 w-full text-center text-sm font-semibold text-blue-700">{t.forgotPassword}</button>}
          ...
          {mode !== 'reset' && <a href="/api/auth/google" className="mt-5 flex w-full items-center justify-center rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">{t.continueWithGoogle}</a>}
          <p className="mt-4 border-t border-slate-100 pt-4 text-center text-xs leading-5 text-slate-500">{t.googleSignInNote}</p>
        </div>
      </div>
    </div>
  );
